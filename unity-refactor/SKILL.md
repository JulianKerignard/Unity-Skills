...
Voir `references/refactor-recipes.md` pour les recettes detaillees avec exemples avant/apres :
- Singleton MonoBehaviour vers SO Service
- God Manager vers services separes
- Magic strings vers constantes/enum
- Coroutine spaghetti vers async/await (mapping `yield` → `Awaitable`)
- References directes vers Event Channel SO
- Old Input vers New Input System (mapping complet des appels)

Charger ce fichier quand un refactoring du plan correspond a une recette.
