...
3. Le skill genere les fichiers de configuration adaptes a partir des templates de `references/build-templates.md`
...
**Coexistence :** `BuildPipeline.BuildPlayer()` classique reste fonctionnel, mais migrer pour les nouveaux projets.

### Etape 3 : Generer le script de build C#

Charger `references/build-templates.md` section "Script de build C#" et creer `Assets/Editor/BuildAutomation.cs`. Adapter selon les plateformes identifiees a l'etape 1 — supprimer les methodes non necessaires.

### Etape 4 : Generer la configuration CI/CD

- **Option A : GitHub Actions** (recommande) — Voir `references/build-templates.md` section "GitHub Actions". Utilise game-ci, support matrix multi-plateformes, cache du dossier `Library/`.
- **Option B : GitLab CI** — Voir `references/build-templates.md` section "GitLab CI". Image `unityci/editor`, jobs par plateforme.

### Etape 5 : Generer .gitignore et .gitattributes

Charger `references/build-templates.md` sections ".gitignore Unity" et ".gitattributes avec LFS". Adapter la liste LFS aux types de binaires presents dans le projet.

### Etape 6 : Pre-build checks

La methode `PreBuildCheck` (incluse dans le template `BuildAutomation.cs`) valide : scenes presentes dans Build Settings, pas d'erreurs de compilation. Verifier aussi en complement : pas de references manquantes dans les prefabs, tests EditMode passent.
